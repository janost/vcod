...
`sv_serverid` is one byte with two fields: `SV_SpawnServer` (cod_lnxded `0x808a598`) adds `0x10` per map load (`movzbl`, so it wraps at 256 and never exceeds a byte, which is why the 1-byte header below round-trips), and `SV_MapRestart_f` (`0x8083eb5..0x8083ece`) bumps the low nibble modulo 16 with `(id & 0xf0) + ((id + 1) & 0xf)`. Both toggle `SNAPFLAG_SERVERCOUNT` (`xorb $4` on the byte at `0x83b67a8`, `0x808a516` and `0x8083eba`). The high nibble skips zero on wrap. VERIFIED: the first `add al, 0x10` is at `0x808a59e` and stores at `0x808a5a3`, a `testb $0xf0` on the same byte sits at `0x808a5ab` with a `jne` to `0x808a5bc`, and a second `add eax, 0x10` at `0x808a5b4` stores at `0x808a5b7`. INFERRED, off that test and its jump: the second add runs only when the first left the high nibble zero, so after the first load the high nibble cycles 1 to 15 and never reads 0. The counter starts at zero, so a freshly started server's first map runs at `0x10`. That is inferred from the add, and it is the value vcod's server uses; the populated capture's `\sv_serverid\18` is a server several loads and restarts in. Round-based gametypes (SD) restart the map between rounds, so the id changes every round.

A map change is announced out of band and the gamestate is pulled, not pushed. VERIFIED: `SV_SpawnServer` holds a client loop at `0x808a2c0..0x808a30c` with a `cmp state, 2` at `0x808a2c6`, a connectionless send of `loadingnewmap\n<map>\n<gametype>` inside it (`NET_OutOfBandPrint`, format string `0x80d5471`), and a call to `0x80c786c(250)` after the loop at `0x808a324`. INFERRED, off that compare and those two addresses: the line goes to every client at `CS_PRIMED` or above and the 250 ms wait follows the last of them, before anything is torn down. VERIFIED: the function contains no call to `SV_SendClientGameState` (`0x8085eec`), and the whole binary has three, all reached from `SV_ExecuteClientMessage` or the `donedl` handler. VERIFIED: it writes configstrings 0 and 1 while `sv.state` still reads 1 (`0x808a8c9`, `0x808a8e1`), and `SV_SetConfigstring` holds a `cmp sv.state, 2` at `0x8089c6c` and a `cmp sv.restarting, 0` at `0x8089c75` ahead of its per-client loop. INFERRED, from those three: neither of those two writes reaches a client, a client learns of the map change from the out-of-band line instead, and the gamestate arrives only after its own next message reaches the new server carrying the old serverId. The full read-out is in `docs/research/cod11-map-cycle.md`.
...
The values the game module puts in those fields, and the filter it applies per
client, are read out of `game.mp.i386.so` in
...
For a **spawned player** it does, and the value is exactly `SHORT2ANGLE(cmd.angles[i] + delta_angles[i])` per axis.

VERIFIED, from the `--save-ads` capture taken on `mp_carentan` under `tdm` with `kar98k_sniper_mp` (`crates/server/tests/fixtures/playerstate/mp_carentan-tdm-ads-sniper.txt`): `delta_angles[1]` reads 24576 and `viewangles[1]` reads -45.0 on every one of its 355 `!trace` lines. Those two pin the wire cmd yaw at 32768: `ANGLE2SHORT(-45)` is 57344, and `57344 - 24576` is 32768. Do not read the capture's `!input yaw=0` as the wire value -- that column records the script's nominal input, and `NetClient::send_frame` (`crates/common/src/net/mod.rs:346`) subtracts `delta_angles` from every axis before the cmd goes out. INFERRED, from the probe's own control flow, that 32768 is what `hold_view_yaw` produces: it holds the view of the first snapshot it saw, which for this run was the spectator's, and the client's subtraction turns that into the wire word.

VERIFIED that the field tracks the cmd rather than pinning the spawn angle, from both motion captures: `mp_carentan-dm-motion.txt` reads 135.0 at every held pose and moves by exactly the turn steps' 60 degrees, to 75.0 at `turn_left` and -165.0 at `turn_right`; `mp_pavlov-dm-motion.txt` is the same shape off a base of 45.0.

VERIFIED that vcod reproduces it: `the_scoped_sight_and_view_match_retail_on_mp_carentan` (`crates/server/tests/playerstate_combat_ab.rs`) compares all three axes against that capture with no tolerance, sample by sample, and the replay sends the capture's own cmd angles.

Two retail *player* captures read `viewangles` 0 and are not counterexamples: `mp_carentan-dm.txt` and `mp_pavlov-dm.txt` were taken by a probe built on vcod's own client, which subtracts `delta_angles` when it builds each cmd, so `cmd.angles + delta_angles` really is zero there. The `--save-ads` and `--save-motion` probes hold the spawn view instead (`hold_view_yaw`, `crates/client/src/probe.rs`), which is why their sum is the spawn yaw. The same subtraction is why `playerstate_ab.rs`'s `check_spawn_shape` still expects zero on every axis and is right to.

The angles live in the `cmd.angles` / `delta_angles` pair regardless: `delta_angles` is set once at spawn as `ANGLE2SHORT(spawn_angle[i]) - cmd.angles[i]` (RTCW `SetClientViewAngle`, `g_client.c:437`), the client subtracts it when building each usercmd, and the client owns its view between snapshots. vcod's own client already does the subtraction (`crates/common/src/net/mod.rs`). `viewangles` is the sum the server hands back; a client that replays at least one usercmd recomputes it and never reads the wire value, which is why the field went unnoticed as unwritten for as long as it did.

vcod's server implements the RTCW scheme: `ClientSim::spectator`/`become_player` set `delta_angles` at spawn from `ANGLE2SHORT(spawn_angle) - cmd.angles`, `ClientSim::step` reads `SHORT2ANGLE(cmd.angles + delta_angles)` rather than the raw cmd angle, and `to_wire` writes `viewangles` for a `pm_type` 0 client and leaves it zero for every other. The first three landed in one commit deliberately: getting one or two without the third rotates the move basis, since the client always subtracts `delta_angles` when it builds a cmd (`crates/common/src/net/mod.rs`) -- a server that sets the delta but keeps reading the raw cmd angle, for instance, has the client compensating for an offset the server never applies, so the client walks at an angle to where it looks. The `viewangles` write is pinned by the pair of tests in `crates/server/src/spectate.rs`, one per path.

An intermission client (`pm_type` 5) is unmeasured: no capture records `viewangles` for one, and vcod leaves it zero there, which is the spectator's answer rather than a measured one. INFERRED.
...
Four fields a walking player carries that a spectator does not, all reproduced by `ClientSim::to_wire`. The first three were measured off `crates/server/tests/fixtures/playerstate/mp_pavlov-dm.txt` and its `mp_carentan` twin; `viewangles` reads 0 in those two for the reason the section above gives and was measured off the sight and motion captures instead:
...
- `viewangles[0..2]`, `SHORT2ANGLE(cmd.angles[i] + delta_angles[i])` per axis, where a spectator's stay 0. VERIFIED off `mp_carentan-tdm-ads-sniper.txt` and both `-motion.txt` captures; the section above has the numbers.
