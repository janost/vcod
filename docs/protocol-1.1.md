...
`sv_serverid` is one byte with two fields: `SV_SpawnServer` (cod_lnxded `0x808a598`) adds `0x10` per map load (`movzbl`, so it wraps at 256 and never exceeds a byte, which is why the 1-byte header below round-trips), and `SV_MapRestart_f` (`0x8083eb5..0x8083ece`) bumps the low nibble modulo 16 with `(id & 0xf0) + ((id + 1) & 0xf)`. Both toggle `SNAPFLAG_SERVERCOUNT` (`xorb $4` on the byte at `0x83b67a8`, `0x808a516` and `0x8083eba`). The high nibble skips zero on wrap: VERIFIED, `SV_SpawnServer` adds a second `0x10` when the first add leaves `id & 0xf0` zero (`0x808a5ab..0x808a5b7`), so after the first load the high nibble cycles 1 to 15 and never reads 0. The counter starts at zero, so a freshly started server's first map runs at `0x10`. That is inferred from the add, and it is the value vcod's server uses; the populated capture's `\sv_serverid\18` is a server several loads and restarts in. Round-based gametypes (SD) restart the map between rounds, so the id changes every round.
