...
For a **spawned player** it does, and the value is exactly `SHORT2ANGLE(cmd.angles[i] + delta_angles[i])` per axis. VERIFIED from the `--save-ads` capture taken on `mp_carentan` under `tdm` with `kar98k_sniper_mp` (`crates/server/tests/fixtures/playerstate/mp_carentan-tdm-ads-sniper.txt`): `delta_angles[1]` 24576 with the probe holding cmd yaw 0, and `viewangles[1]` -45.0 on every one of its 356 samples, which is `SHORT2ANGLE(24576)` normalized to (-180, 180]. VERIFIED that it tracks the cmd rather than pinning the spawn angle, from both motion captures: `mp_carentan-dm-motion.txt` reads 135.0 at every held pose, 75.0 at `turn_left` (cmd yaw -60) and -165.0 at `turn_right` (cmd yaw +60), and `mp_pavlov-dm-motion.txt` is the same shape off a base of 45.0.
