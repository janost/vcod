...
`sv_serverid` is one byte with two fields: `SV_SpawnServer` (cod_lnxded `0x808a598`) adds `0x10` per map load (`movzbl`, so it wraps at 256 and never exceeds a byte, which is why the 1-byte header below round-trips), and `SV_MapRestart_f` (`0x8083ebb`) bumps the low nibble modulo 16 with `(id & 0xf0) + ((id + 1) & 0xf)`. Both toggle `SNAPFLAG_SERVERCOUNT` (`xorb $4` on the byte at `0x83b67a8`, `0x808a516` and `0x8083eba`). The high nibble skips zero on wrap: VERIFIED, `SV_SpawnServer` adds a second `0x10` when the first add leaves `id & 0xf0` zero (`0x808a5ab..0x808a5b7`), so after the first load the high nibble cycles 1 to 15 and never reads 0. The counter starts at zero, so a freshly started server's first map runs at `0x10`. That is inferred from the add, and it is the value vcod's server uses; the populated capture's `\sv_serverid\18` is a server several loads and restarts in. Round-based gametypes (SD) restart the map between rounds, so the id changes every round.

A map change is announced out of band and the gamestate is pulled, not pushed. VERIFIED: `SV_SpawnServer` sends `loadingnewmap\n<map>\n<gametype>` as a connectionless packet (`NET_OutOfBandPrint`, format string `0x80d5471`) to every client whose state is `CS_PRIMED` or above, then sleeps 250 ms before it tears the server down (`0x808a2c0..0x808a329`). VERIFIED: the function contains no call to `SV_SendClientGameState` (`0x8085eec`), and the whole binary has three, all reached from `SV_ExecuteClientMessage` or the `donedl` handler. VERIFIED: it writes configstrings 0 and 1 while `sv.state` still reads 1 (`0x808a8c9`, `0x808a8e1`), and `SV_SetConfigstring` broadcasts only when `sv.state == 2` or `sv.restarting` is set (`0x8089c6c`). INFERRED, from those three: a client learns of the map change from the out-of-band line, and the gamestate arrives only after its own next message reaches the new server carrying the old serverId. The full read-out is in `docs/research/cod11-map-cycle.md`.
