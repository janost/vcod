...
The array blocks are live on a real server, not padding: a spectator's frames carry them on every tick. Skipping them keeps the parse aligned but loses the frame, so vcod records the primitives it consumed (`PlayerState::arrays`) and replays them verbatim; nothing decodes them into fields.
