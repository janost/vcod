//	What survives exitLevel(false): game[], level and a spawned entity.
//	Run by tools/run_probe.sh; every logPrint line is one measurement.
//	exitLevel(false) ends the map; the rotation this probe is run with loads
//	mp_pavlov again under the same gametype, so this file's main() prints the
//	before half on its first pass and the after half on its second.
